...
public class UserListAdapter  extends RecyclerView.Adapter<UserListAdapter.WordViewHolder> {
    ...
    private List<EntityDb> mWords; // Cached copy of words

    UserListAdapter(Context context) { mInflater = LayoutInflater.from(context); }

    @Override
    public WordViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = mInflater.inflate(R.layout.user_view, parent, false);
    ...
    public void onBindViewHolder(WordViewHolder holder, int position) {
        if (mWords != null) {
            EntityDb current = mWords.get(position);
    ...
    void setWords(List<EntityDb> words){
        mWords = words;
